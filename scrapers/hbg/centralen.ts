...
export const info: RestaurantInfo = { id: 'centralen', name: 'Centralen Lounge', autoplayDuration: 10_000 }

export default async function scrape (): Promise<MenuFile[]> {
  const baseUrl = 'https://www.centralenlounge.se'
  const res = await fetch(new URL('/menyer/', baseUrl), {
    headers: {
      accept: 'text/html',
  ...
  const menuHref = $('article.centralen-menu-card:has(.centralen-menu-card__title:contains("Lunchmeny")) a.centralen-menu-card__button').attr('href')
  ...
  const pages = await savePdfImg(pdfRes, 'centralen')
